...
public class GeneticAlgorithm {
    ...
    public static void main(String[] args) {
        ...
        //GeneticAlgorithm knap = new GeneticAlgorithm();
...
//    }
    ...
    private void showOptimalList() {
        ...
        for(int y = 0; y < this.number_of_items; y++) {
            if(optimal_list.substring(y, y + 1).equals("1")) {
                ...
                totalQuality += data.get((y)).getKey();
                totalWeight += data.get((y)).getValue();
            }
        }
        System.out.println("Total quality: " + totalQuality + " Total weight: " + totalWeight);
    }
    ...
    private void getInput(List<Graph.ItemSet> data, int capacity) {
        ...
        for (int i = 0; i < number_of_items; i++){
            value_of_items.add((double)data.get(i).getKey());
            weight_of_items.add((double)data.get(i).getValue());
        }
